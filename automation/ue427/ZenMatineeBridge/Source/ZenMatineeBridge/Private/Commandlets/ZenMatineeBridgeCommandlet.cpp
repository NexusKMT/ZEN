...
)
{
        ...
        for (UEdGraphPin* Pin : Node->Pins)
        {
            ...
            if (Pin->PinType.PinCategory != UEdGraphSchema_K2::PC_Exec)
            {
                for (UEdGraphPin* LinkedPin : Pin->LinkedTo)
                {
                    ...
                    if (Cast<UK2Node_Literal>(LinkedNode) || Cast<UK2Node_CallFunction>(LinkedNode) || Cast<UK2Node_MacroInstance>(LinkedNode))
                    {
    ...
    for (UEdGraphPin* SourceLinked : ControllerEventPin->LinkedTo)
    {
        ...
        if (DestExec == nullptr)
        {
            UE_LOG(LogZenMatineeBridge, Error, TEXT("ZEN_BRIDGE_ERROR clone_exec_pin_missing node=%s"), *(*DestNode)->GetPathName());
            return false;
        }
